...
select 'sales'+CAST(st.StockID as varchar(200)) as id, st.StockName, st.StockID  AS unit_id,
    REPLACE(st.StockName,'Магазин IN UA','Реализация ') as label, COALESCE(SUM(TSumCC_wt),0) as value,  '/mobile' as url,
    'sales' as detail_id
    FROM r_Stocks st
...
select 'returns'+CAST(st.StockID as varchar(200)) as id, st.StockName,st.StockID  AS unit_id,
    REPLACE(st.StockName,'Магазин IN UA','Возвраты ') as label, COALESCE(SUM(TSumCC_wt),0) as value,  '/mobile' as url,
...
UNION ALL
  SELECT 'cash_income'+CAST(st.StockID as varchar(200)) as id, st.StockName, st.StockID AS unit_id,
    REPLACE(st.StockName,'Магазин IN UA ','Выручка НАЛ ') as label, SUM(m.SumCC_wt) as value,  '/mobile' as url,
    'cash' as detail_id
  FROM r_Stocks st
  LEFT JOIN (
...
SELECT 'card_income'+CAST(st.StockID as varchar(200)) as id, st.StockName,  st.StockID AS unit_id,
  REPLACE(st.StockName,'Магазин IN UA ','Выручка ПК ') as label, SUM(m.SumCC_wt) as value,  '/mobile' as url,
   'card' as detail_id
FROM r_Stocks st
LEFT JOIN (
        SELECT sales.StockID, COALESCE(SUM(pays.SumCC_wt), 0) as SumCC_wt
...
REPLACE(st.StockName,'Магазин IN UA ','Выручка прочее ') as label, SUM(m.SumCC_wt) as value,  '/mobile' as url,
 'other' as detail_id
FROM r_Stocks st
LEFT JOIN (
SELECT sales.StockID, COALESCE(SUM(pays.SumCC_wt), 0) as SumCC_wt
